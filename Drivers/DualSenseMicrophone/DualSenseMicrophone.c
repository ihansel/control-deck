...
//	This driver is derived from Apple's MIT-licensed NullAudio sample. It preserves the sample's
//	minimal object model while turning the virtual device into a stereo loopback endpoint:
//	samples written to the output stream are exposed by the input stream at the matching device
//	sample time. ControlDeck writes decoded DualSense Bluetooth microphone audio to the
//	output side; microphone clients read it from the input side.
//
...
//	- a box
//	- a device
//		- uses a fixed 48000 Hz sample rate
//		- provides a rate scalar of 1.0 via hard coding
//	- a single input stream
//		- supports 2 channels of 32 bit float LPCM samples
//		- reads matching sample-time data from the loopback ring, or zero if no writer supplied it
//	- a single output stream
//		- supports 2 channels of 32 bit float LPCM samples
//		- writes data into the loopback ring
//	- controls
//		- master input volume
...
#define										kPlugIn_BundleID				"com.ianhansel.controldeck.audio-driver"
static pthread_mutex_t						gPlugIn_StateMutex				= PTHREAD_MUTEX_INITIALIZER;
static UInt32								gPlugIn_RefCount				= 0;
static AudioServerPlugInHostRef				gPlugIn_Host					= NULL;
static const AudioObjectPropertySelector	kPlugIn_CustomPropertyID		= 'PCst';

#define										kBox_UID						"com.ianhansel.controldeck.dualsense-microphone.box"
static CFStringRef							gBox_Name						= NULL;
static Boolean								gBox_Acquired					= true;

#define										kDevice_UID						"com.ianhansel.controldeck.dualsense-microphone.virtual"
#define										kDevice_ModelUID				"com.ianhansel.controldeck.dualsense-microphone.model"
static pthread_mutex_t						gDevice_IOMutex					= PTHREAD_MUTEX_INITIALIZER;
static Float64								gDevice_SampleRate				= 48000.0;
static UInt64								gDevice_IOIsRunning				= 0;
static const UInt32							kDevice_RingBufferSize			= 16384;
static _Atomic(Float32)						gDevice_RingBuffer[16384 * 2];
static _Atomic(SInt64)						gDevice_RingBufferFrameNumber[16384];
...
static OSStatus	NullAudio_Initialize(AudioServerPlugInDriverRef inDriver, AudioServerPlugInHostRef inHost)
{
	...
	if(gBox_Name == NULL)
	{
		gBox_Name = CFSTR("DualSense Microphone");
	}
...
static OSStatus	NullAudio_PerformDeviceConfigurationChange(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt64 inChangeAction, void* inChangeInfo)
{
	...
	FailWithAction(inChangeAction != 48000, theAnswer = kAudioHardwareBadObjectError, Done, "NullAudio_PerformDeviceConfigurationChange: bad sample rate");
...
static OSStatus	NullAudio_GetPlugInPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyManufacturer:
			...
			*((CFStringRef*)outData) = CFSTR("ControlDeck");
			*outDataSize = sizeof(CFStringRef);
			break;
		...
		case kPlugIn_CustomPropertyID:
			...
			CFShow(*((CFPropertyListRef*)inQualifierData));
			*((CFStringRef*)outData) = CFSTR("DualSense Microphone PlugIn");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetBoxPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyModelName:
			//	This is the human readable name of the maker of the box.
			FailWithAction(inDataSize < sizeof(CFStringRef), theAnswer = kAudioHardwareBadPropertySizeError, Done, "NullAudio_GetBoxPropertyData: not enough space for the return value of kAudioObjectPropertyManufacturer for the box");
			*((CFStringRef*)outData) = CFSTR("DualSense Bluetooth Audio Bridge");
			*outDataSize = sizeof(CFStringRef);
			break;
...
static OSStatus	NullAudio_GetDevicePropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = CFSTR("DualSense Microphone");
			*outDataSize = sizeof(CFStringRef);
			break;
		...
		case kAudioObjectPropertyElementName:
			...
			{
				case 0:
					*((CFStringRef*)outData) = CFSTR("Stereo");
					break;
	...
	*outNumberPropertiesChanged = 0;
	...
	switch(inAddress->mSelector)
	{
		case kAudioDevicePropertyNominalSampleRate:
			...
			FailWithAction(*((const Float64*)inData) != 48000.0, theAnswer = kAudioHardwareIllegalOperationError, Done, "NullAudio_SetDevicePropertyData: unsupported value for kAudioDevicePropertyNominalSampleRate");
...
static OSStatus	NullAudio_GetStreamPropertyData(AudioServerPlugInDriverRef inDriver, AudioObjectID inObjectID, pid_t inClientProcessID, const AudioObjectPropertyAddress* inAddress, UInt32 inQualifierDataSize, const void* inQualifierData, UInt32 inDataSize, UInt32* outDataSize, void* outData)
{
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioObjectPropertyName:
			...
			*((CFStringRef*)outData) = (inObjectID == kObjectID_Stream_Input) ? CFSTR("DualSense Microphone Input") : CFSTR("DualSense Microphone Feed");
			*outDataSize = sizeof(CFStringRef);
			break;
	...
	*outNumberPropertiesChanged = 0;
	...
	switch(inAddress->mSelector)
	{
		...
		case kAudioStreamPropertyPhysicalFormat:
			...
			FailWithAction(((const AudioStreamBasicDescription*)inData)->mSampleRate != 48000.0, theAnswer = kAudioHardwareIllegalOperationError, Done, "NullAudio_SetStreamPropertyData: unsupported sample rate for kAudioStreamPropertyPhysicalFormat");
...
static OSStatus	NullAudio_StartIO(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID)
{
	...
	{
		//	Start the virtual hardware by anchoring the timeline and invalidating all ring frames.
		//	The exact frame-number tags prevent stale samples from a previous IO run being replayed.
		...
		{
			atomic_store_explicit(&gDevice_RingBuffer[(theFrameIndex * 2)], 0.0f, memory_order_relaxed);
			atomic_store_explicit(&gDevice_RingBuffer[(theFrameIndex * 2) + 1], 0.0f, memory_order_relaxed);
			atomic_store_explicit(&gDevice_RingBufferFrameNumber[theFrameIndex], INT64_MIN, memory_order_relaxed);
		}
	}
	else
	{
...
static OSStatus	NullAudio_GetZeroTimeStamp(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, UInt32 inClientID, Float64* outSampleTime, UInt64* outHostTime, UInt64* outSeed)
{
	...
	*outSampleTime = (Float64)(gDevice_NumberTimeStamps * (UInt64)kDevice_RingBufferSize);
...
static OSStatus	NullAudio_DoIOOperation(AudioServerPlugInDriverRef inDriver, AudioObjectID inDeviceObjectID, AudioObjectID inStreamObjectID, UInt32 inClientID, UInt32 inOperationID, UInt32 inIOBufferFrameSize, const AudioServerPlugInIOCycleInfo* inIOCycleInfo, void* ioMainBuffer, void* ioSecondaryBuffer)
{
	//	WriteMix stores output frames in a lock-free ring indexed by the HAL's device sample time.
	//	ReadInput retrieves only frames whose exact sample-time tag is still present. A missing or
	//	overwritten frame becomes silence rather than stale microphone data.
	
	#pragma unused(inClientID, ioSecondaryBuffer)
	
	//	declare the local variables
	OSStatus theAnswer = 0;
	SInt64 theFirstSampleTime;
	...
	FailWithAction(inIOCycleInfo == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done, "NullAudio_DoIOOperation: no IO cycle info");
	FailWithAction(ioMainBuffer == NULL, theAnswer = kAudioHardwareIllegalOperationError, Done, "NullAudio_DoIOOperation: no main buffer");

	if(inOperationID == kAudioServerPlugInIOOperationReadInput)
	{
		FailWithAction(inStreamObjectID != kObjectID_Stream_Input, theAnswer = kAudioHardwareBadObjectError, Done, "NullAudio_DoIOOperation: ReadInput on non-input stream");
		theFirstSampleTime = (inIOCycleInfo->mInputTime.mFlags & kAudioTimeStampSampleTimeValid)
			? (SInt64)inIOCycleInfo->mInputTime.mSampleTime
			: (SInt64)(inIOCycleInfo->mIOCycleCounter * inIOBufferFrameSize);
		Float32* theOutputBuffer = (Float32*)ioMainBuffer;
		for(UInt32 theFrameIndex = 0; theFrameIndex < inIOBufferFrameSize; ++theFrameIndex)
		{
			SInt64 theFrameNumber = theFirstSampleTime + theFrameIndex;
			UInt32 theRingIndex = ((UInt64)theFrameNumber) & (kDevice_RingBufferSize - 1);
			SInt64 theTagBefore = atomic_load_explicit(&gDevice_RingBufferFrameNumber[theRingIndex], memory_order_seq_cst);
			if(theTagBefore == theFrameNumber)
			{
				theOutputBuffer[(theFrameIndex * 2)] = atomic_load_explicit(&gDevice_RingBuffer[(theRingIndex * 2)], memory_order_seq_cst);
				theOutputBuffer[(theFrameIndex * 2) + 1] = atomic_load_explicit(&gDevice_RingBuffer[(theRingIndex * 2) + 1], memory_order_seq_cst);
				SInt64 theTagAfter = atomic_load_explicit(&gDevice_RingBufferFrameNumber[theRingIndex], memory_order_seq_cst);
				if(theTagAfter != theFrameNumber)
				{
					theOutputBuffer[(theFrameIndex * 2)] = 0.0f;
					theOutputBuffer[(theFrameIndex * 2) + 1] = 0.0f;
				}
			}
			else
			{
	...
	{
		FailWithAction(inStreamObjectID != kObjectID_Stream_Output, theAnswer = kAudioHardwareBadObjectError, Done, "NullAudio_DoIOOperation: WriteMix on non-output stream");
		theFirstSampleTime = (inIOCycleInfo->mOutputTime.mFlags & kAudioTimeStampSampleTimeValid)
			? (SInt64)inIOCycleInfo->mOutputTime.mSampleTime
			: (SInt64)(inIOCycleInfo->mIOCycleCounter * inIOBufferFrameSize);
		const Float32* theInputBuffer = (const Float32*)ioMainBuffer;
		for(UInt32 theFrameIndex = 0; theFrameIndex < inIOBufferFrameSize; ++theFrameIndex)
		{
			SInt64 theFrameNumber = theFirstSampleTime + theFrameIndex;
			UInt32 theRingIndex = ((UInt64)theFrameNumber) & (kDevice_RingBufferSize - 1);
			atomic_store_explicit(&gDevice_RingBufferFrameNumber[theRingIndex], INT64_MIN, memory_order_seq_cst);
			atomic_store_explicit(&gDevice_RingBuffer[(theRingIndex * 2)], theInputBuffer[(theFrameIndex * 2)], memory_order_seq_cst);
			atomic_store_explicit(&gDevice_RingBuffer[(theRingIndex * 2) + 1], theInputBuffer[(theFrameIndex * 2) + 1], memory_order_seq_cst);
			atomic_store_explicit(&gDevice_RingBufferFrameNumber[theRingIndex], theFrameNumber, memory_order_seq_cst);
		}
	}
